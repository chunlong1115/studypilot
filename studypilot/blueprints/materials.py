...
def list_materials():
    ...
    from studypilot.models import Question
    qc_map = {}
    for m in materials:
        qc_map[m.id] = Question.query.filter_by(material_id=m.id).count()

    return render_template('materials/list.html',
                           materials=materials, pagination=pagination,
                           type_filter=type_filter, qc_map=qc_map)
...
def upload_material():
    if request.method == 'POST':
        ...
        if mtype not in ('note', 'image'):
            flash('请选择有效的资料类型。', 'danger')
            return render_template('materials/upload.html')

        material = Material(user_id=current_user.id, title=title, material_type=mtype)
...
def edit_material(material_id):
    material = Material.query.filter_by(id=material_id, user_id=current_user.id).first_or_404()
    if material.material_type not in ('note', 'image'):
        flash('仅笔记和图片类型资料支持编辑。', 'danger')
        return redirect(url_for('materials.view_material', material_id=material.id))
